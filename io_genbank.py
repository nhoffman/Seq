...
__version__ = "$Id: io_genbank.py 3364 2010-05-17 20:52:15Z nhoffman $"
        ...
        if in_features:
            if line.startswith(leadingblank):
                ...
                if line.startswith(r'/'):
                    ...
                    # continues the value in (key, value) pair added on
                    # the previous line
                    addto[-1][-1] += line
            else:
                # a feature key
...
if __name__ == '__main__':
    ...
    # seqs = list(read(infile))

    from Bio import SeqIO
    seqs = list(SeqIO.parse(infile, "genbank"))
    seq = seqs[0]
    
    print dir(seq)

    print 'dbxrefs', seq.dbxrefs
    # pprint.pprint(seq.features)

    print 'annotations'
    pprint.pprint(seq.annotations)
    print 'decsription'
    pprint.pprint(seq.description)
    
    seqs = list(_read(infile))
