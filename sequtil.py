...
def get_vnumbers(seqstr, ignore_gaps=True):
...
def reformat_alignment(seqlist, 
	...
	consensus_str = ''.join([consensus(d) for d in tabulated])
	...
	if number_by != 'consensus':
		try:
			vnumbers = get_vnumbers(seqdict[number_by][:], ignore_gaps=False)
		except KeyError:
	...
	if exclude_invariant:
		compare = lambda d: count_subs(d, countGaps=False) >= min_subs	
		...
		if number_by == 'consensus':
			vnumbers = get_vnumbers(consensus_str)
